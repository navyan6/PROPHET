...
def load_tree_probabilities(tree_json_path: Path) -> List[VariantWithProbability]:
    """
    Load HIV variants with tree-derived probabilities.
    
    Each leaf's probability is the product of split probabilities (p_left, p_right)
    along the path from tree root to that leaf. These probabilities are used to
    weight binding affinities in the MOG-DFM objective function.
    
    Args:
        tree_json_path: Path to hadsbm_tree.json (from tree_analysis pipeline)
    
    Returns:
        List of VariantWithProbability objects with tree-computed probabilities
    """
    ...
    # Build tree structure for probability computation
    tree = _build_tree_structure(tree_data)
    
    # Compute probability for each leaf
    variants = []
    for leaf in leaves:
        node_idx = leaf.get("node_index", -1)
        
        if node_idx >= 0 and tree:
            # Use tree-derived probability
    ...
    # Normalize to ensure sum = 1.0 (accounts for numerical errors)
    total_prob = sum(v.probability for v in variants)
    if total_prob > 0:
        variants = [
            VariantWithProbability(name=v.name, sequence=v.sequence, probability=v.probability / total_prob)
            for v in variants
        ]
