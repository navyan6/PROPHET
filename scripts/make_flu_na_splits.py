#!/usr/bin/env python3
"""
Build PROPHET-ready train/test splits for Influenza A H1N1 neuraminidase (NA).

Uses phylogenetic clade holdout (~20% of sequences as test set).

Usage (after download_flu_na.py):
    python scripts/make_flu_na_splits.py --in-dir data/flu_na

Outputs:
    data/flu_na/alignments/train/flu_na_train_aligned.fasta
    data/flu_na/alignments/test/flu_na_test_clade_holdout.fasta
    data/flu_na/trees/train/flu_na_train_tree.nwk
    data/flu_na/trees/train/flu_na_bootstrap_trees.txt
"""
...
def main() -> None:
    ...
    ap.add_argument("--in-dir",  default="data/flu_na", type=Path)
    ...
    in_dir = args.in_dir
    raw_fasta = in_dir / "flu_na_raw.fasta"

    print("\n=== Influenza A H1N1 Neuraminidase (NA) — clade holdout ===")
    seqs = list(SeqIO.parse(str(raw_fasta), "fasta"))
    if len(seqs) < 50:
        sys.exit(f"ERROR: only {len(seqs)} sequences — need at least 50.")

    print(f"  {len(seqs)} NA sequences")

    tmp_raw  = in_dir / "flu_na_tmp.fasta"
    tmp_aln  = in_dir / "flu_na_tmp_aligned.fasta"
    tmp_tree = in_dir / "flu_na_tmp_tree.nwk"
    ...
    setup_target("flu_na", train_raw, test_raw, args.out_dir, args.n_boot, args.seed)

    print("\nNext:")
    print("  rsync -avz data/flu_na/ \\")
    print("    nnori@login.betty.parcc.upenn.edu:/vast/projects/pranam/lab/nnori/hadsbm-hiv/data/flu_na/")
    print("  sbatch run_flu_na_stage1.slurm")
