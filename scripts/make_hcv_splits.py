#!/usr/bin/env python3
"""
Build PROPHET-ready train/test splits for HCV NS3 protease.

Uses HCV genotype 1a sequences with a phylogenetic clade holdout (~20%).
Optionally adds genotype 1b sequences as a cross-subtype holdout.

Usage (after download_hcv_ns3.py):
    python scripts/make_hcv_splits.py --in-dir data/hcv_ns3

Outputs:
    data/hcv_ns3/alignments/train/hcv_ns3_train_aligned.fasta
    data/hcv_ns3/alignments/test/hcv_ns3_test_clade_holdout.fasta
    data/hcv_ns3/trees/train/hcv_ns3_train_tree.nwk
    data/hcv_ns3/trees/train/hcv_ns3_bootstrap_trees.txt
"""
...
def main() -> None:
    ...
    ap.add_argument("--in-dir",  default="data/hcv_ns3", type=Path)
    ...
    # ── HCV NS3: genotype 1a train + clade holdout ───────────────────────────
    print("\n=== HCV NS3 protease (genotype 1a, clade holdout) ===")
    g1a = list(SeqIO.parse(str(in_dir / "hcv_ns3_protease.fasta"), "fasta"))
    if len(g1a) < 30:
        sys.exit(f"ERROR: only {len(g1a)} genotype 1a sequences — need at least 30.")

    print(f"  {len(g1a)} genotype 1a sequences")

    # Align all for tree building
    tmp_raw  = in_dir / "hcv_tmp.fasta"
    tmp_aln  = in_dir / "hcv_tmp_aligned.fasta"
    tmp_tree = in_dir / "hcv_tmp_tree.nwk"
    SeqIO.write(g1a, str(tmp_raw), "fasta")
    ...
    setup_target("hcv_ns3", train_raw, test_raw, args.out_dir, args.n_boot, args.seed)
    ...
    print("  sbatch run_hcv_stage1.slurm")
